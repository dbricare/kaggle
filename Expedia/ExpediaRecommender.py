...
def sparsify(dfdum, dumcols=[], trainsparse=False):
    ...
    # create dummies, converting to string increases df mem size
    # dfdum = pd.get_dummies(dfdum.astype(str)).astype(int)

    # first pass for training data initiates dummy columns
    if len(dumcols) == 0:
        print('')
    ...
    # process test data, check for missing columns and add them as all zeros
        ...
        if len(dfdum.columns) == len(dumcols) and all(dfdum.columns == dumcols):
            pass   # dummy variables of this chunk match previous
        else:   
            # some dummies not present in this chunk, identify and add them to df
        ...
        # return the test data with dummy columns added if necessary
        return dfdum
...
def warmstart(trainiter, estimator, key, fraction, dumcols=[], trainsparse=False):
    ...
    counter = 1
    for chunk in trainiter:
        chunkfrac = stshsp(chunk, fraction, key)
...
#             yc = chunk[key].iloc[idx].copy() # copy
        ...
        if counter > 1:
            if len(yc.unique()) < uniques:
                continue
        Xtrain = train.drop(key, axis=1, inplace=False) # copy
        yc = train[key]
...
def gridtrainfraction(trainiters, estimator, grid, key, gridfrac, trainfrac):
    ''' 
    ...
    Xtrain, y = fractionate(trainiters[0], gridfrac, key)
    ...
    # clfws = warmstart(trainiters[1], estimator, key, trainfrac, dumcols, trainsparse)
    # return clfws, dumcols, trainsparse
    
    # if no warmstart
    # retrieve new fraction of data
    Xtrain, y = fractionate(trainiters[1], trainfrac, key)
...
def trainfraction(trainiters, estimator, key, trainfrac):
    ...
    Xtrain, y = fractionate(trainiters[0], trainfrac, key)
    # dc = []
    # if 'class_weight' in estimator.get_params():
    #     ts = True
    # else:
    ...
    clf = estimator
    clf.fit(Xtrain, y)
    # clf = warmstart(trainiters[1], estimator, key, trainfrac, [], False)
    dumcols = [1]
    trainsparse = False
...
if __name__ == '__main__':
    ...
    trainiter1 = pd.read_csv('train.csv.gz', sep=',', compression='gzip', 
                            chunksize=5000000, usecols=rawcols)
    trainiter2 = pd.read_csv('train.csv.gz', sep=',', compression='gzip', 
                            chunksize=5000000, usecols=rawcols)
    ...
    estimator, rawcols[-1], trainfrac=0.05)
    # clf, dumcols, trainsparse = gridtrainfraction([trainiter1, trainiter2], 
    # estimator, grid, rawcols[-1], gridfrac=0.002, trainfrac=0.02)
